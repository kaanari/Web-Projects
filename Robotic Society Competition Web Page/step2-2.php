<?php
    ...
    function get_soru(){
        global $result;
        $conn=db_connect();
        $stmt=$conn->prepare("SELECT * FROM sorular WHERE hard=1 ORDER BY RAND() LIMIT 1");
        $stmt->execute();
    ...
    function answer($time){
        global $result;
        $point=getpoint(15-$time);
        $conn=db_connect();
        $stmt = $conn->prepare("UPDATE competitor SET q1_id=?, q1_ans=?, q1_time=?, q1_point=? WHERE id=?");
    ...
    if(cookie_control()){
        if($_POST){
            ...
            $time=$interval->s;
            answer($time);
            setcookie("check","step2",time()+3600);
...
<script type="text/javascript">
    ...
    if(timeleft <= 0){
        ...
        document.cookie = "check=step2";
        window.location = 'http://www.kaanari.com.tr';    
    }
    },1000);
</script>
           ...
           <div class="nav">
            ...
            <div style="color:darkred;font-size:50px;margin-top:20px;margin-bottom:0">
                <?php $abc="<script>document.write(timeleft)</script>"; ?><b><span id="countdowntimer">15 </span></b>
            ...
            <div class="content">
                ...
                <center>
                    <h3><span style="color:darkred;">Soru 1</span></h3>
